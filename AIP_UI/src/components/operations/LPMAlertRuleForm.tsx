...
import { LPMAlertRule, TriggerCondition, AlertChannel } from '@/types/alertRules'
...
interface LPMAlertRuleFormProps {
	initialData?: LPMAlertRule | null
	onSubmit: (rule: LPMAlertRule) => void
	onCancel: () => void
}

export const LPMAlertRuleForm = ({ initialData, onSubmit, onCancel }: LPMAlertRuleFormProps) => {
	...
	const [lpmRegion, setLpmRegion] = useState(initialData?.lpmRegion || '')
		...
		const fetchRegions = async () => {
			try {
				...
				if (response.ok) {
					...
					console.log('✅ Loaded regions for Central England COOP:', regionData)
				} else {
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		console.log('🔍 LPM Alert Form Validation:', {
			name: name.trim(),
		...
		if (!lpmRegion) {
			alert('Please select an LPM region')
			return
		}
		...
		console.log('✅ LPM Alert Rule validated successfully:', { 
		...
		<form onSubmit={handleSubmit} className="space-y-6">
			<div className="space-y-2">
				...
				<Input
					...
					placeholder="e.g., Regional Theft Pattern Alert"
					required
				/>
			</div>
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
				<div className="space-y-2">
					<Label htmlFor="region">LPM Region *</Label>
					<Select value={lpmRegion} onValueChange={setLpmRegion} required disabled={loadingRegions}>
			...
			{channels.includes('email') && (
				<div className="space-y-2">
					...
					<p className="text-sm text-muted-foreground">
						Add email addresses for LPM managers who should receive alerts
					</p>
				</div>
			)}
